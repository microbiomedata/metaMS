...
all_data = all_data[['gnps_spectra_id'] + [col for col in all_data.columns if col != 'gnps_spectra_id']]
assert not all_data['gnps_spectra_id'].duplicated().any(), "Duplicate rows found in gnps_spectra_id"
...
assert all_data['ionmode'].isin(['positive', 'negative']).all(), "Ionmode standardization failed"
...
inchikey_df['molecular_formula'] = inchikey_df['inchi'].apply(inchi_to_molecular_formula)
...
print(f"Found {inchi_precursor.shape[0]} unique molecular formula x precursortype combinations with valid InChIKeys")
...
inchi_precursor['precursortype_fixed'] = inchi_precursor['precursortype'].replace(precursor_standardization)
# Add flag if precursortype is in ion_type_dict.keys()
inchi_precursor['precursortype_flag'] = inchi_precursor['precursortype_fixed'].isin(ion_type_dict.keys())
inchi_precursor_summary = inchi_precursor[['precursortype', 'ionmode', 'precursortype_flag']].groupby(['precursortype', 'ionmode', 'precursortype_flag']).size().reset_index(name='count')
# Print out the % of unique unique molecular formula x precursortype combinations that have precursortype_flag == True
print(f"Able to calculate {inchi_precursor[inchi_precursor['precursortype_flag']].shape[0]/inchi_precursor.shape[0]} precursors of molecular formula")
#TODO: Fix remaining precursors and deal with multiply charged species.
...
for row in inchi_precursor.itertuples():
    ...
    try:
        ion_formula = LipidomicsExport.get_ion_formula(molecular_formula, precursor_type)
        mol = MolecularFormula(
            molecular_formula= ion_formula,
            ion_type= "RADICAL",
            ion_charge= ion_mode
        )
...
# ==================================================
curated_data = all_data[['gnps_spectra_id', 'file_name', 'name','peaks', 'precursortype', 'precursormz', 'inchikey', 'ionmode', 'instrumenttype', 'instrument']]
#59k
curated_data_merged = curated_data.merge(inchi_precursor, on=['inchikey','precursortype', 'ionmode'], how='left')
assert len(curated_data_merged) == len(curated_data), "Merge failed, number of rows do not match"

curated_data_merged['precursor_mz_diff'] = curated_data_merged['precursormz'] - curated_data_merged['calculated_precursor_mz']
bad_mz_rows = curated_data_merged[abs(curated_data_merged['precursor_mz_diff']) > 0.1]
print(f"{bad_mz_rows.shape[0]/len(curated_data_merged)*100:.1f}% of rows have bad m/z values for precursor based on the newly calculated m/z values")
curated_data_merged = curated_data_merged[abs(curated_data_merged['precursor_mz_diff']) <= 0.1]
print(f"{curated_data_merged['file_name'].nunique()} remaining file names in the dataset")
...
curated_data_merged = curated_data_merged.drop(columns=['calculated_precursor_mz', 'precursor_mz_diff', 'precursor_mz', 'precursortype_flag'])

# Replace precursortype with the standardized precursortype and keep precursortype as precursortype_original
curated_data_merged['precursortype_original'] = curated_data_merged['precursortype']
...
curated_data_instremove = curated_data_merged[~curated_data_merged['instrumenttype'].isin(bad_instruments)]
print(f"Removed {round((1-curated_data_instremove.shape[0]/curated_data_merged.shape[0])*100, ndigits=1)}% spectra due to low-resolution instruments")
print(f"{curated_data_instremove['file_name'].nunique()} unique file names remain in the dataset after instrument filtering")
print(f"Remaining appropriate databases: {curated_data_instremove['file_name'].unique()}")

# ==================================================
# STEP 9: Summarize the number of unique "emsl compounds" per database
# ==================================================
emsl_compounds = pd.read_csv(os.path.join(lookup_dir, "emsl_compounds.csv"))
curated_data_instremove['emsl_compound_flag'] = curated_data_instremove['refmet_name'].isin(emsl_compounds['refmet_name'].unique())
db_summary = curated_data_instremove[['refmet_name', 'emsl_compound_flag', 'file_name']].drop_duplicates()
db_summary_count = db_summary.groupby(['file_name', 'emsl_compound_flag']).size().reset_index(name='n_refmet_compounds')
# Add back the file_names that 
print('here')


# ==================================================
# STEP 10: Write out a msp file of the curated data
# ==================================================
print(f"Writing out curated MSP file...with {curated_data_instremove.shape[0]/og_spectra_count} of original spectra")
write_to_msp(curated_data_instremove, "curated_msp.msp", msms_col='peaks')
print("Done.")

#get dataframe of just file_name PNNL_MSMLSPlate2MSMS.msp
pnnl_inchikeys = curated_data_merged[curated_data_merged['file_name'] == 'PNNL_MSMLSPlate2MSMS.msp']['inchikey'].drop_duplicates()
pnnl_inchikeys.to_csv(os.path.join(lookup_dir, "pnnl_inchikeys.csv"), index=False)
